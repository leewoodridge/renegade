...
var
  ...
  Cell2: TVideoCell absolute Cell;
  VideoBuf : PVideoBuf;
begin
  InitVideo;
  ...
  // FullScreen
  if (Lines = 1) and (X1 = 0) and (Y1 = 0) and (X2 = ScreenWidth - 1) and (Y2 = ScreenHeight - 1) then
    begin
      // goto lower left corner and send IND
      Write(#27'[s'                            // save cursor position
           +#27'['+IntToStr(ScreenHeight)+';1H'   // X:=1 Y=25
           +#27'D'                             // IND
  ...
  Src := ScreenWidth * (Y1 + Lines) + X1;
  Dst := ScreenWidth * Y1 + X1;
  Len := X2 - X1 + 1;

  for I := Y1 to Y2 - Lines do
    begin
      Move(VideoBuf^[Src], VideoBuf^[Dst], Len * 2);
...
Begin
  ...
  Src := ScreenWidth * (Y2 - Lines) + X1;
  Dst := ScreenWidth * Y2 + X1;
  Len := X2 - X1 + 1;
